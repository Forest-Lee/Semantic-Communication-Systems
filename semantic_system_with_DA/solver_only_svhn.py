        ...
        for iii in range(10):
            ...
            for step in tqdm.tqdm(range(self.train_iters)):
                ...
                if (step + 1) % (self.log_step * 10) == 0:
                    ...
                    path = os.path.join(self.sample_path, 'svhn/woda/sample-%d-m-s.png' % (step + 1))
                    imageio.imwrite(path, merged)
                    print('saved %s' % path)
                    # svhn to mnist
                    merged = self.merge_images(svhn, fake_mnist)
                    merged = imageio.core.image_as_uint(merged)
                    path = os.path.join(self.sample_path, 'svhn/woda/sample-%d-s-m.png' % (step + 1))
                    imageio.imwrite(path, merged)
                    print('saved %s' % path)
                    # all
                    merged = self.merge_images_encoder(svhn, fake_mnist, out_encoder_reshape_data)
                    merged = imageio.core.image_as_uint(merged)
                    path = os.path.join(self.sample_path, 'svhn/woda/sample-%d-s-m-encoder.png' % (step + 1))
                    imageio.imwrite(path, merged)
                    print('saved %s' % path)
